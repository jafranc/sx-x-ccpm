...
template < class SurfaceMesh , class VertexPropertyMap> class Poly_rings
{
protected:
//  //Polyhedron
...
{
	  BOOST_FOREACH(vertex_descriptor vd, vces)
			  vpm[vd]=-1;
}
...
push_neighbours_of(const SurfaceMesh& m,
	...
	BOOST_FOREACH(vertex_descriptor vd, vertices_around_target(start,m) )
			if( vpm[vd] != -1 ){ continue;}//if visited: next
			else
			{
			vpm[vd] = ith;
...
collect_ith_ring(const SurfaceMesh& m, int ith,
	...
	BOOST_FOREACH(vertex_descriptor vd,currentRing)
				push_neighbours_of(m, vd, ith, nextRing, all, vpm);
