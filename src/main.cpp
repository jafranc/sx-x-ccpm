...
int main(int argc, char **argv) {
    ...
    //droplets genertions
        ...
        cimg_interface.get_contactSphere(options_parsed["test"].as<int>(), options_parsed["test-angle"].as<std::vector<double>>(),
    ...
    //workflow options
        ...
        if (ext == "tiff") {
            ...
            if (options_parsed.count("isoval") > 0) {
                ...
                cimg_interface.to_isoValue(output_dir.string() + ("/isoVal_"), options_parsed["isoval"].as<std::vector<int>>(),
                                           options_parsed["ncomponent"].as<int>());
                cimg_interface.to_cc_images(output_dir.string() + ("/isoVal_"), options_parsed["isoval"].as<std::vector<int>>());
                return 0;
            }
